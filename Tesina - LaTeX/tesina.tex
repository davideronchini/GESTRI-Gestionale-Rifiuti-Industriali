...
\begin{figure*}[!ht]
    \centering
    \includegraphics[width=0.39\textwidth]{DiagrammaMacchineStatiAttivita}
\end{figure*}

\clearpage
\newsubsection{Macchine a Stati: Mezzo}
Questo diagramma di stato, una volta corretto, modella il ciclo di vita completo di un mezzo. Il mezzo inizia nello stato Disponibile e passa allo stato Occupato quando viene assegnato a un'attività. Al termine dell'attività, il mezzo ritorna allo stato Disponibile, pronto per una nuova assegnazione. Se la condizione di scadenzaRevisione < oggi viene verificata come vera, il mezzo entra nello stato di Manutenzione, per poi tornare nuovamente Disponibile una volta che i lavori sono stati completati. In ogni momento, se il mezzo viene eliminato dal sistema, il suo stato si conclude, come rappresentato dal simbolo di Exit nel diagramma UML, che indica la terminazione del ciclo di vita dell'oggetto.

\begin{figure*}[!ht]
    \centering
    \includegraphics[width=0.39\textwidth]{DiagrammaMacchineStatiMezzo}
\end{figure*}
